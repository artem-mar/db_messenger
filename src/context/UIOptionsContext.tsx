...
const UIReducer = (options: TOptionsMap, { name, value }: IUIOption): TOptionsMap =>
  new Map([...options, [name, value]])
...
export const UIOptionsProvider = ({ children }: Props) => {
  ...
  const value = useMemo(() => ({ UIOptions: computed, setUIOption }), [computed])
